...
let options2 = [
  { value: 'realtime', label: 'Real-Time Outbreaks', disabled: false, selected: true },
  { value: 'historical', label: 'Historical Case Counts', disabled: false },
];
...
function updateMap() {
  ...
  .on('mouseover', function(d) {
    ...
    d3.select(this)
      ...
      <strong><i>${formatDate(d.date)}</strong> | ${casesText(d.cases)}</i></div>
  ...
  .on('click', function(d) {
    ...
    if (stickyTooltip === this) {
      ...
      ${d.place_name}</div>
      <div style="margin-bottom:8px;">
      <strong><i>${formatDate(d.date)}</strong> | ${d.cases} cases</i></div>
